      ...
      <div className="print-area" style={{ padding: '60px', fontFamily: '"Times New Roman", Times, serif', maxWidth: '800px', margin: '0 auto', background: '#fff', boxShadow: '0 0 20px rgba(0,0,0,0.3)', minHeight: '1000px', position:'relative' }}>
        ...
        <div style={{ lineHeight: '1.8', fontSize: '1.1rem', textAlign: 'justify' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', margin:'20px 0', background:'#f8fafc', padding:'20px', border:'1px solid #ddd' }}>
            <div><strong>SPONSOR:</strong><br/>{mySponsorship.name}<br/>{mySponsorship.email}</div>
            <div style={{textAlign:'right'}}><strong>ORGANIZER:</strong><br/>{event.user?.collegeName || "College Committee"}<br/>Event: {event.title}</div>
          </div>

          <div style={{ textAlign: 'center', margin: '30px 0', padding: '20px', border: '3px double #000' }}>
          ...
          <ol style={{ marginLeft: '20px' }}>
            {/* 👇 UPDATED CLAUSE AS REQUESTED */}
            <li style={{ marginBottom: '10px', color:'#b91c1c', fontWeight:'bold' }}>
                The Sponsor agrees to transfer the full amount within <span style={{textDecoration:'underline'}}>3 Business Days</span> from today. Failure to do so will result in the immediate cancellation of this deal.
            </li>
            <li style={{ marginBottom: '10px' }}>The Organizer agrees to provide branding/promotion as discussed upon receipt of funds.</li>
            <li>This document serves as a binding proof of the sponsorship pledge.</li>
          </ol>

          {/* 👇 SPONSOR'S COMMENTS */}
          {mySponsorship.comment && (
              <div style={{marginTop:'20px', padding:'15px', background:'#fffbeb', border:'1px solid #fcd34d'}}>
                  <strong>📝 Additional Notes from Sponsor:</strong><br/>
