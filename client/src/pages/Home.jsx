  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", backgroundColor: '#f8fafc', overflowX: 'hidden' }}>
      ...
      <motion.div 
        ...
        style={{ marginTop: '-50px', display: 'flex', justifyContent: 'center', padding: '0 20px', position: 'relative', zIndex: 10 }}
      >
        <div style={{ background: 'white', padding: '15px', borderRadius: '15px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', display: 'flex', gap: '10px', maxWidth: '800px', width: '100%', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            />
            <button onClick={handleSearch} style={{ padding: '15px 30px', background: '#0f172a', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem' }}>
              Search
            </button>
        </div>
      </motion.div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '50px', padding: '60px 20px', flexWrap: 'wrap', color: '#64748b' }}>
        {['🔒 100% Secure Payment', '✅ Verified Organizers', '⚡ 24 Hour Approval'].map((item, index) => (
          <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontWeight: '600', fontSize: '1.1rem' }}>
            {item}
          </div>
        ))}
      </div>
      ...
      {/* 🚀 TRENDING EVENTS */}
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px 20px 100px' }}>
              ...
              >
                ...
                <div style={{ padding: '25px' }}>
                  <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                    <span style={{ background: '#ecfdf5', color: '#059669', padding: '5px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold' }}>
                      Needed: ₹{event.budget}
                    </span>
                    <span style={{ fontSize: '0.9rem', color: '#64748b' }}>📍 {event.location}</span>
                  </div>
                  
                  <h3 style={{ fontSize: '1.4rem', fontWeight: 'bold', color: '#1e293b', marginBottom: '10px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {event.title}
                  </h3>
                  <p style={{ color: '#64748b', fontSize: '0.95rem', lineHeight: '1.6', height: '45px', overflow: 'hidden' }}>
                    {event.description}
                  </p>
        ...
        <div style={{textAlign:'center', marginTop:'50px'}}>
            <Link to="/all-events" style={{padding:'12px 30px', border:'2px solid #cbd5e1', borderRadius:'30px', textDecoration:'none', color:'#475569', fontWeight:'bold', transition:'0.3s'}}>
                View All Events
            </Link>
        </div>
      </div>
