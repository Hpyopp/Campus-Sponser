...
const registerUser = asyncHandler(async (req, res) => {
  ...
  // Send Email (Log error if fails but don't stop)
  try {
      await sendEmail({ email: user.email, subject: 'Verify Account', message: `Your OTP is: ${otp}` });
  } catch (err) {
      console.log("Email Failed to Send (Dev Mode)");
  }
...
// 3. FORGOT PASSWORD (Returns OTP in Response for Green Box)
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  // Try sending email, but ignore error for Dev Mode
  try {
      await sendEmail({ email: user.email, subject: 'Reset Password Code', message: `Your Password Reset Code is: ${otp}` });
  } catch (error) { 
      console.log("Email sending failed, using Dev Mode Green Box"); 
  }

  // 👇 IMPORTANT: Sending 'debugOtp' to Frontend
...
const getMe = asyncHandler(async (req, res) => { 
    const user = await User.findById(req.user.id); 
    if (user) { 
        res.status(200).json({ _id: user._id, name: user.name, email: user.email, role: user.role, isVerified: user.isVerified, verificationDoc: user.verificationDoc, companyName: user.companyName }); 
    } else { res.status(404); throw new Error('User not found'); } 
});
