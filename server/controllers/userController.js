...
const verifyRegisterOTP = asyncHandler(async (req, res) => {
  ...
  if (user && user.otp === otp.toString().trim()) {
      // Admin hai toh direct verify, warna False (Doc upload ka wait karo)
...
// 3. LOGIN USER (Allows login to check status)
...
// 4. UPLOAD DOCUMENT (Sets status to Pending)
...
// 5. GET CURRENT USER (For Syncing)
const getMe = asyncHandler(async (req, res) => {
  ...
  if (user) {
    res.status(200).json({
        ...
        role: user.role,
        isVerified: user.isVerified,
        verificationDoc: user.verificationDoc, // Important for Lock Screen
...
// 👇 8. ADMIN: REVOKE/REJECT USER (CRITICAL FIX)
const unverifyUser = asyncHandler(async (req, res) => {
  // Reject karte waqt DOC ko NULL karo, tabhi user re-upload kar payega
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  sendEmail({ email: user.email, subject: 'Reset Password Code', message: `Your Password Reset Code is: ${otp}` })
    .catch(err => console.log("Email Dev Mode: Skipped waiting"));
    
  res.json({ message: 'OTP Generated', debugOtp: otp });
});
