...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const otp = Math.floor(100000 + Math.random() * 900000).toString();
  user.otp = otp; user.otpExpires = Date.now() + 10 * 60 * 1000; await user.save();
  sendEmail({ email: user.email, subject: 'Login OTP', message: `Your OTP is: ${otp}` }).catch(err => console.log("Email Failed"));
  res.status(200).json({ message: 'OTP Ready', debugOtp: otp });
});
...
const uploadDoc = asyncHandler(async (req, res) => {
  ...
  if (user) {
    user.verificationDoc = req.file.path; user.isVerified = false; await user.save();
    res.status(200).json({ message: 'Uploaded', docUrl: req.file.path });
  } else { res.status(404); throw new Error('User not found'); }
});

// 6. GET CURRENT USER (AUTO SYNC KE LIYE ZAROORI 🟢)
const getMe = asyncHandler(async (req, res) => {
  ...
  if (user) {
      res.status(200).json({
          ...
          email: user.email,
          role: user.role,
          isVerified: user.isVerified, // Frontend ye check karega
...
module.exports = { 
  registerUser, loginUser, verifyLogin, verifyRegisterOTP, uploadDoc, 
  getMe, // 👈 Export hona chahiye
